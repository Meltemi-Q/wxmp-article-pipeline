...
- 评论区互动行（如「对了，你最想 X？评论区聊聊👇」）**默认位置 = 正文结尾收尾处**。
- **文末 Hashtag 规则**：互动行之后可自然加入 3-5 个精炼话题标签（如 `#AI #Agent #Grok #Cursor #马斯克`），帮助微信平台算法抓取与话题聚合，严禁堆砌无意义标签。
- 签名更新为「**我是宇龙，专注 AI Agent 场景应用落地。**」只允许出现一条；重推前 `assert content.count("我是宇龙") <= 1`。
- 用户没明示要签名时，默认使用当前标准定位签名。

### 🧯 公众号合规边界：不写自动化创作/发布教程

遇到公众号文章，尤其是写给公开平台的文章，先判断是否会落到“非真人自动化创作/发布教程”风险里。
...
- 个人使用测试
- AI 辅助整理素材
- 图片识别、表格整理、客户聊天归纳、会议待办
- 人工判断、人工选题、人工修改
- 哪里省事，哪里还得人盯着

如果用户目标读者是企业老板、小白、运营/销售负责人：
...
2. 看用户语料：本机先跑 `python3 scripts/voice_fewshot.py --query "<本篇主题>" --n 8`，从朋友圈/社群/历史公众号捞跟本篇相关的原句。写完再跑 `python3 scripts/voice_match.py --target article.md`，按文章类型对照口气。只学句式，不搬隐私事实。`persona_style.md` 是摘要，不能代替检索。有「发布版 vs 推送前草稿」就对照手改，别只看 AI 长稿。自动化验收走 `aigc_check.sh`，朱雀网页只抽查，不要为刷分改口气。
