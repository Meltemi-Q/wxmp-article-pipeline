...
6. **QC + 过检 + 交付**：先套手改操作（段尾句号混合、图注无句号、标题砍第二分句），再跑 QC（含 stylometry）。可选 `scripts/zhuque_check.sh`。交付时写清原话来源：口述 / 截图气泡 / 几乎没有。几乎没有的，标注「建议发布前快速手改一遍」。默认 `write_only`，用户明说「直接推」才推草稿箱。
